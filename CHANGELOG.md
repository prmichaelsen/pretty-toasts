...
- Deferred `handleRemove()` call to next tick using `setTimeout(..., 0)`
- Prevents setState during render in Toast component auto-dismiss logic

### Technical Details
- Modified `src/components/Toast.tsx` line 146
- Wrapped `handleRemove()` in `setTimeout` to defer execution
- No breaking changes - all functionality preserved

## [3.0.3] - 2026-02-15

### Changed
- Updated warning toast gradient to orange→purple for better visual appeal
- Gradient: orange-500 → purple-500
- Progress bar: orange-400 → purple-400

## [3.0.2] - 2026-02-15

### Fixed
- Fixed toast positioning bug where toasts would stack above empty space after other toasts were dismissed
- Toasts now properly reposition and drop to bottom after dismissals
- Updated `calculatePositions` to exclude exiting toasts from position calculations

### Technical Details
- Exiting toasts now maintain their position during exit animation
- Remaining toasts smoothly transition to new positions
- Modified `ToastContainer.tsx` to handle position recalculation correctly
